...
for i = 1 : length(image_filenames)
    ...
    [X, probabilities] = extract_cnn_features(cnn, { fullfile(root_folder, image_filenames{i}) });
    % save them in a MAT file
    current_filename = image_filenames{i};
    save(fullfile(output_path, strcat(current_filename(1:end-3), 'mat')), 'X', 'probabilities');
end