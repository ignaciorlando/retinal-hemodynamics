...
for i = 1 : data_partition.NumTestSets
    ...
    for j = 1 : length(ks)
        ...
        if add_cnn_features
            [coeff, X_cnn, ~] = pca(retrieve_cnn_features( cnn_features_path, training_cnn_samples ));
            X = cat(2, X, X_cnn);
        end 
        ...
        if add_cnn_features
            X_val_cnn = retrieve_cnn_features( cnn_features_path, validation_cnn_samples ) * coeff;
    ...
    if add_cnn_features
        X_test_cnn = retrieve_cnn_features( cnn_features_path, test_cnn_samples ) * coeff;
